...
class Api::V1::MultiAccountsController < Api::BaseController
  ...
  def consume
    ...
    unless grant
      render json: { error: 'Authorization code not found or already used' }, status: :unauthorized
      return
    end

    unless grant.application_id == application.id
      render json: { error: 'Authorization code does not match OAuth application' }, status: :unauthorized
      return
    end
    ...
    if token_response.is_a?(Doorkeeper::OAuth::ErrorResponse)
      ...
      error_message = error_body[:error_description] || error_body[:error] || 'Failed to exchange authorization code for token. Please try again.'
    ...
    begin
      ...
      Rails.logger.error("[MultiAccount] Failed to mark token #{access_token&.id} with multi-account flags: #{e.message}")
    end
    ...
    render json: { error: 'An error occurred while adding the account. Please try again.' }, status: :internal_server_error
  end
  ...
  def refresh_token
    ...
    Rails.logger.error(e.backtrace.join("\n"))
    render json: { error: 'An error occurred while refreshing the token.' }, status: :internal_server_error
  end
end
